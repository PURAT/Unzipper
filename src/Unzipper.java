...
public class Unzipper {
    ...
    private final static String ZIP_FILE = "D:\\my_tmp\\test.zip";
    private final static String DESTINATION_DIRECTORY = "D:\\my_tmp";
    private final static String ZIP_EXTENSION = ".zip";
    private final static String ZIP_EXTENSION2 = ".tar";

    public static void main(String[] args) {
        System.out.println("Trying to unzip file " + ZIP_FILE);
        Unzipper unzip = new Unzipper();
        if (unzip.unzipToFile(ZIP_FILE, DESTINATION_DIRECTORY)) {
            System.out.println("Successfully unzipped to the directory "
            ...
            while (zipFileEntries.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    if (entry.getName().toLowerCase().endsWith(ZIP_EXTENSION) ||
                            entry.getName().toLowerCase().endsWith(ZIP_EXTENSION2)) {
                        String zipFilePath = destDirectory.getPath() + File.separatorChar + entry.getName();

                        unzipToFile(zipFilePath, zipFilePath.substring(0, zipFilePath.length() - ZIP_EXTENSION.length()));
                    }
                }
            }
            bufIS.close();
